...
from ..exts.forms import AccessForm, CarForm
from ..exts import db
from .models import CarType, Visitor, UnloadingTime, Unloading
    ...
    if car_form.validate_on_submit():
        ...
        try:
            new_unloading = Unloading(supplier=car_info.get("supplier"), name=car_info.get("delivery_contact"), phone=car_info.get("phone"), license_number=car_info.get(
                "license_number"), car_type_id=car_info.get("car_type"), unloading_time=datetime.strptime(car_info.get("scheduled_unloading_time"), "%Y-%m-%d %H:%M:%S"))
...
def unloading():
    ...
    date = request.args.get("date", datetime.date(datetime.now()))
    page = request.args.get("page", 1)
    pagination = db.session.query(Unloading.supplier, Unloading.name, Unloading.phone, Unloading.license_number, CarType.name, Unloading.unloading_time).join(CarType, Unloading.car_type_id == CarType.id).filter(db.cast(
