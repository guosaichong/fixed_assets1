...
@public_bp.route("/")
def index():
    industry=db.session.query(News).filter(News.news_type_id==1).order_by(News.id.desc()).limit(5)
    company=db.session.query(News).filter(News.news_type_id==2).order_by(News.id.desc()).limit(5)
    notice=db.session.query(News).filter(News.news_type_id==3).order_by(News.id.desc()).limit(5)
    return render_template("public/index.html",industry=industry,company=company,notice=notice)
...
def publish_news():
    ...
    news=News(user_id=user_id,title=title,content=news_content.encode(),news_type_id=int(news_type))
    db.session.add(news)
    db.session.commit()
